...
int LinhaCSV(char *line, char fields[][200], int maxFields) {
    ...
    int count = 0;
Show shows[MAX_lISTA];

bool inserirInicio(Show s) {
    if (count >= MAX_lISTA) return false;
    // Move todos para a direita
...
bool inserirPos(Show s, int pos) {
    if (count >= MAX_lISTA || pos < 0 || pos > count) return false;
