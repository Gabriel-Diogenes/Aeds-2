...
void iniciarFila(FilaFlexivel *f) {
    f->inicio = f->fim = NULL;
    f->tamanho = 0;
}

bool filaVazia(FilaFlexivel *f) {
    return f->inicio == NULL;
}

void inserirFila(FilaFlexivel *f, Show s) {
    ...
    for (int i = 0; i < f->tamanho; i++) {
        soma += tmp->elemento.release_year;
        tmp = tmp->prox;
    }
    int media = soma / f->tamanho;
    printf("[Media] %d\n", media);
}

Show removerFila(FilaFlexivel *f) {
    Show removido;
    if (filaVazia(f)) {
