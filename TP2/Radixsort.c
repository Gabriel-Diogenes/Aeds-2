...
void countingSortByDigit(Show arr[], int n, int exp) {
    Show output[n];
    int count[10] = {0};

    for (int i = 0; i < n; i++) {
        int index = (arr[i].release_year / exp) % 10;
        count[index]++;
    }
    ...
    for (int i = 0; i < n;) {
        int j = i + 1;
        while (j < n && output[i].release_year == output[j].release_year) j++;

        for (int m = i; m < j - 1; m++) {
            for (int k = m + 1; k < j; k++) {
                comparacoes++;
                if (strcmp(output[m].title, output[k].title) > 0) {
                    Show temp = output[m];
                    output[m] = output[k];
                    output[k] = temp;
                }
...
void escreverLog(double tempo) {
    FILE *log = fopen("692997_radixsort.txt", "w");
    if (log) {
